...
// Add a default member user for easy dev testing
users.set(2, {
  id: 2,
  name: 'Test Member',
...
const generateToken = (user: User): string => {
  return jwt.sign({ id: user.id, email: user.email, role: user.role }, JWT_SECRET, { expiresIn: '5h' })
}
...
export default defineMock([
  {
    ...
    body: async ({ body }) => {
      ...
      // Look for existing user with matching credentials
      let user = Array.from(users.values()).find(u => u.email === email && u.password === password)

      // Dev convenience: auto-create user if not found so sign-in never fails after Vite restart
      if (!user && email && password) {
        ...
        user = { id, name: email.split('@')[0], email, password, role: 'MEMBER' }
        users.set(id, user)
      }
